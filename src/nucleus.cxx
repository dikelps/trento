...
namespace trento
{
  ...
  NucleusPtr Nucleus::create(const VarMap& var_map, std::size_t index)
  {
    // W-S params ref. in header
    // XXX: remember to add new species to the help output in main() and the readme
                        
    const std::string &species = var_map["projectile"].as<std::vector<std::string>>().at(index);
    double nucleon_dmin = var_map["nucleon-min-dist"].as<double>();
